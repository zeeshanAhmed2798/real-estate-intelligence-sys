...
st.title("🏡 Location Recommender System")
...
def recommend_societies_with_scores(society_name, top_n=5):
    ...
    # index
    idx = society_df[society_df["Society"] == society_name].index[0]
    ...
    # dataframe (UI wording only)
    recommendations_df = pd.DataFrame(
        {
            "Recommended Location": society_df["Society"].iloc[society_indices].values,
            "Similarity Score": [round(i[1], 3) for i in sim_scores],
        }
    )
...
# ---------------- UI ----------------
selected_society = st.selectbox("Select Location", sorted(society_df["Society"].unique()))

# ---------------- BUTTON ----------------
if st.button("Recommend"):
    recommendations = recommend_societies_with_scores(selected_society)

    st.subheader("🏘 Similar Locations")
